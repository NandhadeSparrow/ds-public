...
def plot_crosstabs(df, target_col, suitable_cols):
    ...
    for col in suitable_cols:
        ...
        crosstab.plot(kind='bar', stacked=0, figsize=(5, 3))
        plt.title(f'Distribution of Credit Scores by {col}')
...
def plot_counts(df, target, cols, num_cols=3):
    """
    Plots count plots of categorical columns in a DataFrame in a grid layout.
    ...
    for i, col in enumerate(cat_cols):
        ...
        sns.countplot(df[col], hue=df[target], ax=axes[row, col_mod], order=df[col].value_counts().index)

        # Set title and labels
        axes[row, col_mod].set_title(col)
        axes[row, col_mod].set_ylabel('Count')

        # Rotate x-axis labels for better readability
        plt.setp(axes[row, col_mod].get_xticklabels(), rotation=45, ha="right")
    ...
    return df.columns  # Return the column names (optional)
